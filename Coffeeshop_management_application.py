...
def create_cute_cat_reset(parent):
    ...
    canv.create_polygon(25, 38, 40, 15, 45, 38, fill=ear_pink) 
    canv.create_polygon(70, 40, 80, 10, 100, 40, fill=calico_orange, outline="black", width=2)
    canv.create_polygon(75, 38, 80, 15, 95, 38, fill=ear_pink) 
    ...
    # Whiskers
    canv.create_line(10, 60, 30, 63, fill="black")
    canv.create_line(10, 70, 30, 67, fill="black")
    canv.create_line(90, 63, 110, 60, fill="black")
...
def refresh_data():
    ...
    menu_names = inv_df['name'].tolist()
    
    # Update the dropdown menu in Tab 3
    if menu_names:
        option_men3.set(menu_names[0])
        menu_sale_dropdown['menu'].delete(0, 'end')
        for name in menu_names:
            menu_sale_dropdown['menu'].add_command(label=name, command=tk._setit(option_men3, name))
    else:
        option_men3.set("No Items")
...
def update():
    ...
    if not selected:
        messagebox.showwarning("Selection", "Select an item from the list")
        return
...
def generate_report():
    ...
    if df.empty:
        messagebox.showinfo("Report", "No sales yet!")
        return
    ...
    messagebox.showinfo("Report Ready", "Saved to calico_business_report.csv")
...
notebookTab.add(tab4, text="Update =ᗢ=")
notebookTab.add(tab5, text="Delete =ᗢ=")
notebookTab.add(tab6, text="Report =ᗢ=")
notebookTab.pack(expand=1, fill="both")

# Tab 1
ttk.Label(tab1, text="Coldbrew Calico Coffeehouse ₊˚⊹♡", font=("Arial", 15)).pack(pady=10)
Menu_OptionsView1 = ttk.Treeview(tab1, columns=("id","name","category","price","active"), show="headings", height=5)
for c in ("id","name","category","price","active"): Menu_OptionsView1.heading(c, text=c.title())
Menu_OptionsView1.pack(pady=5)
salesView1 = ttk.Treeview(tab1, columns=("id","parent_id","date","quantity","total"), show="headings", height=5)
for c in ("id","parent_id","date","quantity","total"): salesView1.heading(c, text=c.title())
salesView1.pack(pady=5)
...
tk.Button(tab2, text="Add Item", command=add_item, bg="#E2A263").pack(pady=5)
Menu_OptionsView2 = ttk.Treeview(tab2, columns=("id","name","category","price","active"), show="headings", height=8)
for c in ("id","name","category","price","active"): Menu_OptionsView2.heading(c, text=c.title())
Menu_OptionsView2.pack()

# Tab 3 (The problematic one)
tk.Label(tab3, text="Select Item:").pack()
option_men3 = tk.StringVar(value="Select Item")
# FIX: Don't .pack() on the same line as variable assignment
menu_sale_dropdown = tk.OptionMenu(tab3, option_men3, "None")
menu_sale_dropdown.pack()

tk.Label(tab3, text="Date (YYYY-MM-DD):").pack()
date_entry = tk.Entry(tab3)
date_entry.insert(0, "2026-04-30")
date_entry.pack()
tk.Label(tab3, text="Quantity:").pack()
quantity_entry3 = tk.Entry(tab3); quantity_entry3.pack()
tk.Button(tab3, text="Add Sale", command=add_sale, bg="#E2A263").pack(pady=5)
salesView3 = ttk.Treeview(tab3, columns=("id","parent_id","date","quantity","total"), show="headings", height=8)
...
tk.Button(tab4, text="Update Selected", command=update).pack(pady=5)
Menu_OptionsView4 = ttk.Treeview(tab4, columns=("id","name","category","price","active"), show="headings", height=8)
for c in ("id","name","category","price","active"): Menu_OptionsView4.heading(c, text=c.title())
Menu_OptionsView4.pack()

# Tab 5
tk.Button(tab5, text="Delete Selected Item (Deactivate)", command=delete, bg="red", fg="white").pack(pady=20)
Menu_OptionsView5 = ttk.Treeview(tab5, columns=("id","name","category","price","active"), show="headings", height=15)
for c in ("id","name","category","price","active"): Menu_OptionsView5.heading(c, text=c.title())
Menu_OptionsView5.pack()

# Tab 6
tk.Button(tab6, text="Generate Business Report", command=generate_report).pack(pady=10)
